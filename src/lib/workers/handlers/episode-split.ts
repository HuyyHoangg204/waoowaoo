...
export async function handleEpisodeSplitTask(job: Job<TaskJobData>) {
  ...
  await reportTaskProgress(job, 20, {
    ...
    stageLabel: 'Preparing episode parameters',
    displayMode: 'detail',
  })
    ...
    for (let attempt = 1; attempt <= MAX_EPISODE_SPLIT_ATTEMPTS; attempt += 1) {
      try {
        ...
        await reportTaskProgress(job, 60, {
          ...
          stageLabel: attempt === 1 ? 'Parsing episodes' : `Parsing episodes (Retry ${attempt - 1})`,
          displayMode: 'detail',
        })
        ...
        await reportTaskProgress(job, 80, {
          ...
          stageLabel: 'Matching episode boundaries',
          displayMode: 'detail',
        })
        ...
        for (let idx = 0; idx < splitEpisodes.length; idx += 1) {
          ...
          // Trust the actual markers in the original text.
          // Don't throw errors based on LLM's hallucinated startIndex or endIndex.
  ...
  if (!episodes) {
    throw lastError || new Error('Failed to match episode boundaries')
  }

  await reportTaskProgress(job, 96, {
    stage: 'episode_split_done',
    stageLabel: 'Smart text split complete',
    displayMode: 'detail',
  })
