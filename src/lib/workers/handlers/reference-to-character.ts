...
export async function handleReferenceToCharacterTask(job: Job<TaskJobData>) {
  ...
  const characterName = readString(payload.characterName) || 'New Character - Default'
  ...
  await reportTaskProgress(job, 15, {
    ...
    stageLabel: 'Preparing reference image conversion',
    displayMode: 'detail',
  })
  ...
  if (extractOnly) {
    await reportTaskProgress(job, 45, {
      ...
      stageLabel: 'Extracting reference description',
      displayMode: 'detail',
    })
    ...
    await reportTaskProgress(job, 96, {
      ...
      stageLabel: 'Reference description extracted',
      displayMode: 'detail',
    })
    return {
      success: true,
  ...
  await reportTaskProgress(job, 35, {
    ...
    stageLabel: 'Generating character sheet',
    displayMode: 'detail',
  })
