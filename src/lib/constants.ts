 ...
 * All logic that distinguishes primary vs. secondary appearances must reference this constant.
 * Secondary appearances start from PRIMARY_APPEARANCE_INDEX + 1.
 */
...
// Option list for config pages (derived from ASPECT_RATIO_CONFIGS)
...
// Banana model resolution options (only for grid storyboard; single-shot generation is fixed at 2K)
...
// Seedance batch model list (uses idle GPU time, cost reduced by 50%)
...
// Models that support audio generation (Seedance 1.5 Pro only, including batch variant)
...
export const FIRST_LAST_FRAME_MODELS = [
  ...
  { value: 'doubao-seedance-1-5-pro-251215-batch', label: 'Seedance 1.5 Pro (First-Last/Batch) Save 50%' },
  { value: 'doubao-seedance-1-0-pro-250528', label: 'Seedance 1.0 Pro (First-Last Frame)' },
  { value: 'doubao-seedance-1-0-pro-250528-batch', label: 'Seedance 1.0 Pro (First-Last/Batch) Save 50%' },
  { value: 'doubao-seedance-1-0-lite-i2v-250428', label: 'Seedance 1.0 Lite (First-Last Frame)' },
  { value: 'doubao-seedance-1-0-lite-i2v-250428-batch', label: 'Seedance 1.0 Lite (First-Last/Batch) Save 50%' },
...
export const ART_STYLES = [
  ...
  },
  {
    ...
    label: '3D Animation',
    preview: '3D',
    promptZh: '皮克斯迪士尼风格3D卡通动画，圆润可爱的角色造型，柔和明亮的色彩，细腻的光影效果，适合儿童的温馨画面风格，高质量3D渲染',
    promptEn: 'Pixar Disney style 3D cartoon animation, rounded cute character design, soft bright colors, delicate lighting and shadows, warm child-friendly visual style, high-quality 3D rendering.'
  }
]
...
/**
 * Get the art style prompt from ART_STYLES constant.
 * This is the single source of truth for style prompts.
 * 
 * @param artStyle - Style identifier, e.g. 'realistic', 'american-comic'
 * @returns The corresponding style prompt, or empty string if not found
 */
...
// Character design system suffix (always appended to prompt, not shown to user) - left facial close-up + right three-view
export const CHARACTER_PROMPT_SUFFIX = 'Character design sheet, the image is divided into two areas: [Left area] approximately 1/3 width, a frontal close-up of the character (if human, show full face; if animal/creature, show the most recognizable frontal form); [Right area] approximately 2/3 width, three-view character sheet arranged horizontally (from left to right: front full body, side full body, back full body), all three views at the same height. Pure white background, no other elements.'

// Location image generation system suffix (quad-view disabled, generates a single scene image)
...
// Remove location system suffix from prompt (for display to user)
export function removeLocationPromptSuffix(prompt: string): string {
  if (!prompt) return ''
  return prompt.replace(LOCATION_PROMPT_SUFFIX, '').replace(/，$/, '').replace(/,\s*$/, '').trim()
}

// Add location system suffix to prompt (for image generation)
export function addLocationPromptSuffix(prompt: string): string {
  // Return original prompt if suffix is empty
...
/**
 * Build character introduction string (sent to AI to help it understand pronouns and character references)
 * @param characters - Character list, must include name and introduction fields
